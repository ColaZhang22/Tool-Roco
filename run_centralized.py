...
from prompting import SelforganizationCentral_Prompter, SelfOrganizationCentralLLMResponseParser
...
class LLMRunner:
    def __init__(
        ...
        if self.llm_comm_mode == "decentralized":
            ...
            self.parser = SelfOrganizationCentralLLMResponseParser(
                self.env,
                llm_output_mode,
                self.env.robot_name_map,
                self.response_keywords,
        ...
        if llm_comm_mode == "centralized":
            ...
            self.prompter = SelforganizationCentral_Prompter(
                env=self.env,
                parser=self.parser,
                feedback_manager=self.feedback_manager,
                max_tokens=1024,
    ...
    def one_run(self, run_id: int = 0, start_step: int = 0, skip_reset = False, prev_llm_plans = [], prev_response = None, prev_actions = None):
        ...
        if self.llm_comm_mode == "centralized" or "auto_organization":
            history = []
        elif self.llm_comm_mode == "decentralized":
            history = {}   
        for step in range(start_step, start_step + self.max_runner_steps):
            # Turn level 每一次重制system prompt
            task_name = self.env.env_name
            # if self.llm_comm_mode == "centralized":
            ...
            if step == start_step and (len(prev_llm_plans) > 0 or prev_response is not None):
                ...
                if not ready_to_execute or current_llm_plan is None or len(current_llm_plan) == 0:
            ...
            for i, plan in enumerate(current_llm_plan):
                ...
                if prev_actions is not None:
                    ...
                    if plan_success:
                        history.append({"role": "system", "content": f"[REWARD] At step{step}, Successfully parse the plan and successfully execute the planned path."})
                        ...
                        history.append({"role": "system", "content": f"[REWARD] At step{step}, Plan failed to execute:{reason}, you need to replan a different path parameters and avoid Collision with any objective."})
                        # save the log for evaluation
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_runs", '-nruns', type=int, default=4)
    parser.add_argument("--run_name", "-rn", type=str, default="aamas_pack_gpt_4o_mini_cen_org_re")
    parser.add_argument("--tsteps", "-t", type=int, default=6)
    parser.add_argument("--task", type=str, default="pack")
    parser.add_argument("--output_mode", type=str, default="action_and_path", choices=["action_and_path"])
    parser.add_argument("--comm_mode", type=str, default="centralized", choices=["centralized", "auto_organization"])
    parser.add_argument("--control_freq", "-cf", type=int, default=10)
    parser.add_argument("--skip_display", "-sd", action="store_false")
    parser.add_argument("--direct_waypoints", "-dw", type=int, default=3)
    parser.add_argument("--num_replans", "-nr", type=int, default=4)
    parser.add_argument("--cont", "-c", action="store_true")
    parser.add_argument("--load_run_name", "-lr", type=str, default="pack")
    ...
    parser.add_argument("--llm_source", "-llm", type=str, default="gpt-5-nano")
    logging.basicConfig(level=logging.INFO)
