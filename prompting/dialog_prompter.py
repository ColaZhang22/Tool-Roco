...
class DialogPrompter:
    ...
    def compose_system_prompt(
        ...
        2. Describe the HISTORY of the previous rounds.
        3. Describe the Current Environment State.
        4. Describe the Agent and answer format.
        5. Describe the tool list.
        """
        import json
        tools_file = open("/Users/zhangke/Desktop/robot-collab-toolusing/prompt_template/agent/Alice/tools.json","r", encoding="utf-8")
        tools =  json.load(tools_file)
            
        system_prompt_file = open("/Users/zhangke/Desktop/robot-collab-toolusing/prompt_template/task/task_cabinet/cabinet_prompt.json","r", encoding="utf-8")
        system_prompt_pool =  json.load(system_prompt_file)
        ...
        Answer_format = system_prompt_pool["ANSWER_FORMAT"]
        ...
        system_prompt = [
            {"role": "system", "content": Task_desc + Cooperation_decs},
            {"role": "system", "content": str(tools) + round_history + execute_feedback},
            {"role": "user", "content": Agent_prompt + Answer_format},
        ]   
...
{parsed_str} Re-format to strictly follow [Action Output Instruction]!"""
    ...
    def compose_agent_prompt(self, agent_name: str, n_calls: int) -> str:
        """
        Create a specific agent prompt.
        """
    ...
    def get_tools(self, agent_name: str) -> List[Dict[str, Any]]:
        """
    ...
    def prompt_one_dialog_round(
        ...
        while n_calls < self.max_calls_per_round:
            for agent_name in self.robot_agent_names:
                ...
                system_prompt = self.compose_system_prompt( obs, agent_name, chat_history=chat_history, current_chat=agent_responses, feedback_history=feedback_history) 
                agent_prompt = self.compose_agent_prompt(agent_name, n_calls)
    ...
    def query_once(self, system_prompt, user_prompt, tools,  max_query):
        response = None
        usage = None   
        # print('======= system prompt ======= \n ', system_prompt)
        # print('======= user prompt ======= \n ', user_prompt)
