...
import json
class SelfOrganizationCentralLLMResponseParser:
    """
    ...
    def parse_connect_action(self, agent_name, obs, robot_state, tool_info):
        ...
        for candidate_agent in candidate_agents:
            if candidate_agent not in self.active_agents:
                ...
                return False, "No Existed Agent Name can connect", wait_plan
    ...
    def central_parse(self, obs: EnvState, response: str, active_agents):
        """ 
        ...
        tool_match = re.search(r"<tool calling>(.*?)</tool calling>", response, re.DOTALL)
        if not tool_match:
            return False, "No <thought> section found in response", []
        ...
        try:
            ...
            return False, 'Tool format does not follow json format, modify it into correct format', []
        ...
        for single_agent_name, single_tool_info in tool_dict.items():
            ...
            try:
                ...
                return False, f"Failed to parse action for {single_agent_name}, check the action format.", []
...
if __name__ == "__main__":
    ...
    env = CabinetTask()
    obs = env.reset()
    parser = CentralizedLLMResponseParser(env,  robot_agent_names=dict(ur5e_robotiq="Alice", panda="Bob", ur5e_suction="Chad",),direct_waypoints=3,)
    
    succ, path_plans = parser.central_parse(
        obs,
        response="""
            <thought>
            In the previous attempts, Alice tried to PICK the cup using paths that resulted in collisions with the table and IK failures. This suggests that the direct approach from her current position to the cup is not feasible due to spatial constraints near the table and cabinet. To address this, I will decompose the task for Alice: first, use MOVE to reposition her gripper to a location that is higher and further away from the table and cabinet, then in the next round, attempt to PICK the cup from this new, safer position. This should help avoid collisions and IK failures.

            Bob must continue to WAIT and hold the right door handle to keep the right door open, as the cup is still inside the cabinet. Chad is holding the mug at the mug coaster and does not need to move.

            Assignment:
            - Alice: MOVE to a new position that is higher and further away from the table and cabinet, preparing for a safe PICK in the next round.
            - Bob: WAIT, holding the right door handle.
            - Chad: WAIT, holding the mug at the mug coaster.
            </thought>
            <tool calling>
            {
            "Alice": {
