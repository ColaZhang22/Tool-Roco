...
class LLMResponseParser:
    ...
    #ROCO-TOOL addtional tools
    def parse_grasp_action(self, agent_name, obs: EnvState, robot_state: RobotState, action_desp: str,): 
        current_pose = np.array(robot_state.ee_pose)
        ...
        # inhand = self.adjust_inhand_names(agent_name, obs, robot_state, action_desp)
    ...
    #ROCO2-TOOL addtional tools
    def parse_release_action(self, agent_name, obs: EnvState, robot_state: RobotState, action_desp: str,): 
        current_pose = np.array(robot_state.ee_pose)
        ...
        return True, "", [dict(robot_name=agent_name, ee_targets=current_pose, ee_waypoints=waypoints, tograsp=None, inhand=inhand, action_strs="GRASP",)]
...
if __name__ == "__main__":
    from rocobench.envs import MujocoSimEnv, EnvState, SortOneBlockTask, CabinetTask 
    env = CabinetTask()
    obs = env.reset()
