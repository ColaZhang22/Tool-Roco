...
class CentralziedPrompter:
    ...
    def compose_system_prompt(self, obs: EnvState) -> str:  
        """
        Compose the system prompt for the four parts:
        1. Describe the whole task.
        2. Describe the Current Environment State.
        3. Describe the Agent and the tool list of this agent.
        4. Describe the answer and tool using format.
        """
        system_prompt_pool =  json.load(open(f"prompt_template/task/task_cabinet/cabinet_prompt.json","r", encoding="utf-8"))

        # Task_desc = system_prompt_pool["CABINET_TASK_CONTEXT"]
        # Cooperation_decs = system_prompt_pool["CABINET_TASK_CHAT_PROMPT"]
        ...
        for agent_name in self.robot_agent_names:
            ...
            system_prompt.append({"role": "user", "content": self.env.get_agent_prompt(obs, agent_name, tools)})
    ...
    def prompt_one_round(self, system_prompt, obs: EnvState, save_path: str = "", result_dir: str="", step: int = 0, history: List=[]) -> Tuple[bool, Optional[List[LLMPathPlan]], List[Dict]]: 
        ...
        for i in range(self.num_replans):
            ...
            parse_succ, parsed_str, llm_plans = self.parser.central_parse(obs, centralized_response) 
            ...
            if parse_succ:  
                ...
                for j, llm_plan in enumerate(llm_plans): 
                    ...
                    if not ready_to_execute:
                        ...
                        result_log = {"Env":f"{self.env.env_name}", "prompt_tokens":prompt_tokens, "completion_tokens":completion_tokens, "Step": step, "Replan": i, "Prompt":system_prompt, "Response":centralized_response ,"Tools Calling":1, "Valid Parameter":0,"Execution": 0, "Reason":env_feedback}
                ...
                # result_log = {"Env":f"{self.env.env_name}", "Step": step, "Replan": i, "Tools Calling":1, "Valid Parameter":0,"Reason":env_feedback}
            ...
            if ready_to_execute: 
                ...
                result_log = {"Env":f"{self.env.env_name}", "prompt_tokens":prompt_tokens, "completion_tokens":completion_tokens, "Step": step, "Replan": i, "Prompt":system_prompt, "Response":centralized_response,"Tools Calling":1, "Valid Parameter":1,  "Execution": 0 ,"Reason":""}
                break  
            else:
    ...
    def prompt_one_centralized_round(self, system_prompt, obs, replan_idx: int = 0, save_path: str ='data/', step: int = 0):
        ...
        system_prompt = self.compose_centralized_answer_format(system_prompt)
