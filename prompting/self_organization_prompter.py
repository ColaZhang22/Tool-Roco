...
class Selforganization_Prompter:
    """
    ...
    def prompt_one_round(self, system_prompt, obs: EnvState, save_path: str = "", result_dir: str="", step: int = 0, history: dict={}) -> Tuple[bool, Optional[List[LLMPathPlan]], List[Dict]]: 
        ...
        # 在self organization中,我们需要维护一个active agent list 和 all agent list。
        # system prompt 包括所有agent的history历史记录
        # active prompt
        for i in range(self.num_replans):
            ...
            system_prompt, centralized_response, current_state, prompt_tokens, completion_tokens = self.prompt_self_organization_round(system_prompt, obs, replan_idx=i,save_path=save_path, step=step)
            ...
            parse_succ, parsed_str, llm_plans = self.parser.self_organization_parse(obs, centralized_response, self.active_agents) 
           
            # 判断是否parse成功
            if parse_succ: 
                # 如果是内部工具，直接进入下一轮
                if parsed_str == "Interior Tool":
                    result_log = {"Env":f"{self.env.env_name}", "active_agents": self.active_agents,  "prompt_tokens":prompt_tokens, "completion_tokens":completion_tokens, "Step": step, "Replan": i, "Prompt":system_prompt, "Response":centralized_response ,"Tools Calling":1, "Valid Parameter":0,"Execution": 0, "Reason":"Interiror Tool Success"}
    ...
    def compose_current_agent_pool(self, system_prompt: List[Dict], agent_name):
        system_prompt_pool =  json.load(open(f"prompt_template/task/{self.env.env_name}/system_prompt.json","r", encoding="utf-8"))
        agentgroup_format = system_prompt_pool["SELFORGANIZATION_AGENT_TEAM_STATE"]
        inactive_agents =[]
        # detect active agents
        for id in self.robot_agent_names:
            if id not in self.active_agents:
                inactive_agents.append(id)
        agentgroup_format = agentgroup_format.format(agent_name=agent_name, active_robots=', '.join(self.active_agents), inactive_robots=', '.join(inactive_agents))

        system_prompt[agent_name].append({"role": "user", "content": agentgroup_format})
        return system_prompt
    ...
    def compose_one_agent_tools(self, system_prompt: List[Dict], agent_name: str) -> List[Dict]:
        ...
        # for agent_name in self.robot_agent_names:
        tools =  json.load(open(f"prompt_template/agent/{agent_name}/tools.json","r", encoding="utf-8"))
        system_prompt[agent_name].append({"role": "system", "content": f"You are {agent_name}, it has a tool list and are:" + json.dumps(tools)})
        return system_prompt
    ...
    def prompt_self_organization_round(self, system_prompt, obs, replan_idx: int = 0, save_path: str ='data/', step: int = 0):
        """
        ...
        """
        active_prompt = {k: system_prompt[k] for k in self.active_agents}
        total_response = {}
        current_state = {}
        prompt_tokens_sum = 0
        ...
        for agent_name in self.active_agents:
            n_calls = 0      
            active_prompt = self.compose_one_agent_state(obs, active_prompt, n_calls, agent_name)  
            active_prompt = self.compose_one_agent_tools(active_prompt, agent_name)  
            active_prompt = self.compose_current_agent_pool(active_prompt, agent_name)
            active_prompt = self.compose_decentralized_answer_format(active_prompt, agent_name)
            while n_calls < self.max_calls_per_round:
                current_state[agent_name] = "".join(d["content"] for d in system_prompt[agent_name][-3:-1])
                response , prompt_tokens, completion_tokens = self.query_once(active_prompt[agent_name], max_query=3,)
                prompt_tokens_sum += prompt_tokens
                completion_tokens_sum += completion_tokens
                active_prompt[agent_name].append({"role": "assistant", "content": response})
                # Avoid NONE response
                if response != "" or response is not None:
                    total_response[agent_name]=response
                    break
            # save current prompt and answer
        self.save_prompt_response(save_path, replan_idx, n_calls, active_prompt)
        return active_prompt, total_response, current_state, prompt_tokens_sum, completion_tokens_sum
