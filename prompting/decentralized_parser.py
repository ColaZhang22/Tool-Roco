...
import json
class DecentralizedLLMResponseParser:
    """
    ...
    def parse_single_line(
        ...
        if tool_info["tool"] == "CONNECT_AGENT" or tool_info["tool"] == "DISCONNECT_AGENT": tool_info["tool"] = "WAIT"
    ...
    def decentral_parse(self, obs: EnvState, response: str):
        """ 
        ...
        # Extract the thought text
            ...
            # assert thought_match, "No <thought> section found in response"
            if not thought_match:
                return False, f"The faliure reaseon from {agent_name} No <thought> section found in response", []
            ...
            try:
                ...
                return False, f'The faliure reaseon from {agent_name} Tool format does not follow json format, modify it into correct format', []
            # check if the response contains all the robot names
        
        assert len(tool_dict) == len(self.robot_agent_names), f"Response must contain exactly one ACTION for each robot, and must contain all keywords: {self.response_keywords}"
        ...
        for single_agent_name, single_tool_info in tool_dict.items():
            ...
            try:
                ...
                return False, f"The faliure reaseon from {single_agent_name} Parsing error, please check the action format", []
            
            if not parse_succ:
                return False, reason, []
            all_kwargs[single_agent_name] = plan_kwargs
        
        #nAlign action length into same length for all agents
        # max_len = max([len(kwargs) for kwargs in all_kwargs.values()])
        # same_len_kwargs = dict()
        # for agent_name, kwargs in all_kwargs.items():
        #     if len(kwargs) == max_len:
        ...
        # kwargs = dict(agent_names=[], ee_targets={}, ee_waypoints={}, parsed_proposal="", action_strs=dict(), inhand=dict(), tograsp=dict(), return_home=dict(),)
        # for agent_name, plan_kwarg_list in all_kwargs.items():    
        #     kwargs['agent_names'].append(agent_name)
        #     for key, val in plan_kwarg_list[0].items():
        #         if key == 'robot_name':
        #             continue
        #         kwargs[key][agent_name] = val
        #     if 'return_home' not in plan_kwarg_list:
        #         kwargs['return_home'][agent_name] = False
        # # check if waypoint lengths match!
        # waypoint_lens = [len(waypoints) for waypoints in kwargs['ee_waypoints'].values()]
        # if not all([waypoint_lens[0] == _len for _len in waypoint_lens]):
        #     return False, 'Planned PATH must have exact same number of steps for all agents', []
        # path_plans=[LLMPathPlan(**kwargs)]
...
if __name__ == "__main__":
    ...
    env = CabinetTask()
    obs = env.reset()
    parser = DecentralizedLLMResponseParser(env,  robot_agent_names=dict(ur5e_robotiq="Alice", panda="Bob", ur5e_suction="Chad",),direct_waypoints=3,)
    response = {"Alice":"""<thought> The task requires both cabinet doors to be open before picking the cup inside. Currently, both the left and right doors are closed. As Alice, I can reach the left door handle, mug, and cup. My gripper is empty and positioned at (0.39, -0.07, 0.46), which is close to the left door handle at (-0.4, 0.3, 0.7). The first step is to open the left door. According to my tool list, I should use the OPEN tool to open the left door handle. I will plan a horizontal curved path from my current gripper position to the left door handle, ensuring the path avoids collision with Bob and Chad, who are both far from my path. The path will be evenly spaced and will end at the left door handle. </thought><tool calling>{"Alice": {
    "tool": "OPEN",
    "parameters": { "handle": "left_door_handle",
      "path": [
        [0.39, 0.03, 0.46],
        [0.13, 0.18, 0.58],
        [-0.13, 0.26, 0.67],
...
</tool calling>""","Bob":"""  <thought>
The task requires both cabinet doors to be open before the cup inside can be picked. The right door is currently closed, and I (Bob) am the only robot who can reach the right_door_handle. My gripper is empty and positioned at (-0.15, 1.30, 0.62), which is far from the right_door_handle at (-0.4, 0.6, 0.7). There are no obstacles or other robots in my direct path. The most efficient next step is for me to move my gripper towards the right_door_handle in preparation for opening it. I will use the MOVE tool to approach the handle, planning a smooth path that avoids collision with Alice and Chad, and stays clear of the table and objects. This sets up the next action to OPEN the right door.
</thought>
<tool calling>
{
  "Bob": { 
...
</tool calling>""", "Chad": """<thought>
The right cabinet door is currently closed, and I, Chad, am able to reach the right_door_handle. My gripper is at (1.09, 1.23, 0.45), and the right_door_handle is at (-0.4, 0.6, 0.7). To enable access to the cup inside the cabinet, the right door must be opened. The most efficient action is to use the OPEN tool to move my gripper to the right_door_handle and open it. I will plan a horizontal curved path that avoids collision with Alice and Bob, and is evenly spaced between my current position and the handle.
</thought>
<tool calling>
{
  "Chad": {
    ...
    succ,parsed_str, path_plans = parser.decentral_parse(
        obs,
        response=response)