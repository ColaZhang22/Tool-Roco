...
class PlannedPathPolicy:
    ...
    def parse_llm_plan_to_qpos(
        ...
        for tstep, ee_poses in enumerate(ee_waypoints_list):  
            # print(f"原本路径点:{ee_poses}")
            # physics_cp = physics.copy(share_model=True)
            attempt_qpos_dict, attempt_full_qpos = self.ik_ee_poses_to_qpos(physics, ee_poses) 
            # verfiy if OK.
            # qpos = attempt_full_qpos[self.rrt_planner.all_joint_idxs_in_qpos]
            # verfiy_pose = self.rrt_planner.forward_kinematics_all(q=qpos, return_ee_pose=True) 
            # for robot_name, pose in verfiy_pose.items():
            #     target_pose = ee_poses[robot_name]
            #     error = np.linalg.norm(np.array(pose.position) - np.array(target_pose.position))
            #     print(f"{robot_name} FK误差: {error:.4f} m")
    ...
    def plan_qpos(self, physics):
        ...
        #     )
        #     ee_poses = self.rrt_planner.forward_kinematics_all(q=qpos, return_ee_pose=True)  
        #     for robot_name, pose in physics_cp.items():  
        #         x, y, z = pose.position  
        #         print(f"中间路径点 - {robot_name}: ({x:.3f}, {y:.3f}, {z:.3f})")
        ...
        # breakpoint()
        # ee_poses = self.rrt_planner.forward_kinematics_all(q=joints_start_qpos, return_ee_pose=True)  
        # for robot_name, pose in ee_poses.items():  
        #     x, y, z = pose.position  
        #     print(f"开始路径点 - {robot_name}: ({x:.3f}, {y:.3f}, {z:.3f})")
        # for i, qpos in enumerate(self.joints_qpos_waypoints):  
        #     ee_poses = self.rrt_planner.forward_kinematics_all(q=qpos, return_ee_pose=True)  
        #     for robot_name, pose in ee_poses.items():  
        #         x, y, z = pose.position  
        #         print(f"中间路径点 {i} - {robot_name}: ({x:.3f}, {y:.3f}, {z:.3f})")
        ...
        # for i, qpos in enumerate(path_ls):  
        #     ee_poses = self.rrt_planner.forward_kinematics_all(q=qpos, return_ee_pose=True)  
        #     for robot_name, pose in ee_poses.items():  
        #         x, y, z = pose.position  
        #         print(f"路径点 {i} - {robot_name}: ({x:.3f}, {y:.3f}, {z:.3f})")
    ...
    def map_qpos_to_ctrl(self, physics, qpos: np.ndarray, include_inhand: bool = True) -> Dict[str, np.ndarray]:
        ...
        # for robot_name, obj_info in self.inhand.items(): 
        #     if obj_info is not None: 
        #         grasp_val = obj_info["inhand_val"]
        #         # Get the coordinate of objective
        #         obj_site = obj_info["inhand_site_name"]
        #         site_xpos = physics.data.site(obj_site).xpos
                
        #         #读取手中物体状态
        #         weld_id = obj_info["weld_id"]
        #         constraint_states = self.env.physics.data.eq_active[weld_id]
        #         # Get the index of control and set the force of control. If val = 0, no Force, else has Force.
        #         grasp_idxs.append(self.robots[robot_name].grasp_idx)  
        #         grasp_ctrl_val = self.robots[robot_name].get_grasp_ctrl_val(grasp=(grasp_val > 0))
        #         grasp_vals.append(grasp_ctrl_val)
        # eq_active_idxs = []
        # eq_active_vals = []
        # # if self.use_weld and self.inhand is not None:
        #     # for robot_name, obj_info in self.inhand.items():
        # for robot_name_to, obj_info_to in self.tograsp.items():
        #     # if obj_info != []: 
        #     if obj_info_to is not None:
        #     # if robot_name == robot_name_to and obj_info == obj_info_to:
        #         weld_id = obj_info_to["weld_id"]
        #         eq_active_idxs.append(int(weld_id))
        #         # eq_active_vals.append(np.arrary(self.env.physics.data.eq_active[weld_id]))
        #         eq_active_vals.append(int(1))
    ...
    def get_grasp_action(
        ...
        for robot_name, obj_info in self.tograsp.items(): 
            # Define the objective need to grasp
            if obj_info is not None: 
                ...
                grasp_ctrl_val = self.robots[robot_name].get_grasp_ctrl_val(grasp=(grasp_val > 0))
                grasp_vals.append(grasp_ctrl_val)
                # Get if has touch the object
                if grasp_val > 0:
                    pose = target_ee_poses[robot_name]
                    robot_ee_pos = pose.position 
                    dist = np.linalg.norm(site_xpos - robot_ee_pos)
                    if dist > 0.1:
                        print(f"WARNING: robot {robot_name} location {robot_ee_pos}, handle location {site_xpos} end effector distance: {dist} is too far from object {obj_info['obj_name']}. Weld does not start.")   
                        # if start weld, make task easy.
                        # if self.use_weld and obj_info.get("weld_id", None) is not None:
                        #     # both adhesion and eq_active is turned off
                        #     weld_id = obj_info["weld_id"]
                        #     eq_active_idxs.append(weld_id)
                        #     assert int(grasp_val) in [int(0), int(1)], f"grasp_val should be integer 0 or 1 when using weld"
                        #     eq_active_vals.append(int(0))
                    else: 
                        print(f"Grasp target and start weld")
                if self.use_weld and obj_info.get("weld_id", None) is not None:
