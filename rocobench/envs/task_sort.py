...
class SortOneBlockTask(MujocoSimEnv):
    ...
    def get_one_agent_objective_prompt(self, obs: EnvState, agent_name: str):
        ...
        agent_prompt = agent_prompt.format(agent_name=agent_name, bin_name=bin_name, other_robots=', '.join(other_robots), cube_name=cube_name , reachable_panels=reachable_panels, agent_state=agent_state, cube_states = cube_states)
    ...
    def get_agent_prompt(self, obs, agent_name, include_response_instructions=True):
        ...
        f_ = open(f"./prompt_template/task/{self.env_name}/system_prompt.json", "r", encoding="utf-8")
        agent_prompt = json.load(f_)["AGENT_PROMPT_TEMPLATE"]
        agent_prompt.format(agent_name=agent_name, bin_name=bin_name,other_robots=', '.join(other_robots), cube_name=cube_name , reachable_panels=reachable_panels, agent_state=agent_state)
...
# EXECUTE\nNAME Alice ACTION WAIT\nNAME Bob ACTION PICK blue_square PLACE panel3\nNAME Chad WAIT
    ...
    def describe_cube_state(self, obs: EnvState, cube_name: str):
        ...
        x, y, z = top_site.xpos
        cube_desp = f"Location of {cube_name} is [{x:.3f},{y:.3f},{z:.3f}]"
        for slot_name, pos in self.bin_slot_pos.items():
            if np.linalg.norm(pos[:2] - cube_state.xpos[:2]) < self.align_threshold:
                ...
                if (cube_name=="blue_square" and  slot_name=="panel2") or (cube_name=="pink_polygon" and  slot_name=="panel4") or (cube_name=="yellow_trapezoid" and  slot_name=="panel6"):
                    cube_desp += f"This cube {cube_name} has been correctly place in right panel {slot_name}."
                else:
                    cube_desp += f"This cube {cube_name} currently in wrong panel and still need to place in target panel by next action."
                break 
        if len(cube_desp) == 0:
