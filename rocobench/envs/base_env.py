...
class MujocoSimEnv:
    ...
    def reset(self, keyframe_id=0, home_pos=None, reload=False, filepath=None):
        ...
        #     gname = self.physics.model.geom(i).name
        #     if gname and "mug" in gname:
        #         geom_pos = self.physics.data.geom_xpos[i]  # geom 世界坐标
        #         body_id = self.physics.model.geom(i).bodyid
        #         body_pos = self.physics.data.xpos[body_id]  # body 世界坐标
        #         print(f"{gname} | geom_pos: {geom_pos} | body_pos: {body_pos}")
    ...
    def get_contact(self) -> Dict:
        ...
        # NOTE: eq_active is repleced by eq_active0 in dm_control
    ...
    def step(self, action: SimAction, verbose: bool = False) -> Tuple[EnvState, float, bool, dict]:
        ...
        # if action.eq_active_idxs is not None:
        #     self.physics.model.eq_active0[action.eq_active_idxs] = action.eq_active_vals
        #     self.physics.data.eq_active[action.eq_active_idxs] = action.eq_active_vals
        #     self.physics.forward()  # 确保每次修改后都 forward
        # constraint_states = self.physics.data.eq_active
        # print(f" Weld active? {constraint_states}")
        ...
        for step in range(self.sim_forward_steps): 
            ...
            if eq_active_idxs is not None and len(eq_active_idxs) > 0:
                self.physics.model.eq_active[action.eq_active_idxs] = action.eq_active_vals
        ...
        # robots= self.get_sim_robots()
        # for robot_name in robots:  
        #     real = self.get_robot_name(robot_name)
        #     robot_state = getattr(next_obs, real)  
        #     x, y, z = robot_state.ee_xpos  
        #     print(f"Step - {robot_name} gripper: ({x:.3f}, {y:.3f}, {z:.3f})")
