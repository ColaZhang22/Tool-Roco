...
class SweepTask(MujocoSimEnv):
    ...
    def get_target_quat(self, agent_name, target_name) -> Optional[np.ndarray]:
        ...
        if agent_name =="Alice":
            xmat = self.physics.data.site(f"{target_name}_top").xmat.copy().reshape(3,3)
            rot_z_180 = np.array([[-1, 0, 0], [0, -1, 0], [0, 0, 1]])
            xmat = xmat @ rot_z_180
            ret = mat_to_quat(xmat)  
            return ret
    ...
    def describe_cube_state(self, obs, cube_name: str = "red_cube") -> str: 
        ...
        return cube_desp
    def get_agent_ojective_prompt(self, obs: EnvState, agent_name: str):
        ...
        agent_prompt = agent_template.format(agent_name=agent_name, other_robot=other_robot, agent_state=agent_state,cube_states=cube_states,tool=tool, instruction =instruction)
