...
class PackGroceryTask(MujocoSimEnv):
    ...
    def describe_obs(self, obs: EnvState):
        ...
        for slot_name, slot_xpos in self.bin_slot_xposes.items():
            x, y, z = slot_xpos
            full_desp += f"{slot_name}: ({x + 0.1:.3f}, {y:.3f}, {z:.3f})\n"
    ...
    def get_agent_objective_prompt(self, obs, agent_name):   
        ...
        slot_desp = "\n".join(
            [
                f"{slot_name}: ({x+0.15:.2f}, {y:.2f}, {z:.2f})" for slot_name, (x,y,z) in self.bin_slot_xposes.items()
            ]
            )
        ...
        agent_prompt = agent_template.format(agent_name=agent_name, other_robot=other_robot, slot_desp=slot_desp, object_desp=object_desp, robot_desp=robot_desp, table_height=table_height,)
        return agent_prompt

    def get_one_agent_objective_prompt(self, obs, agent_name):   
        robot_name = self.get_robot_name(agent_name)
        other_robot = "Alice" if agent_name == "Bob" else "Bob"
        object_desp = "\n".join([self.describe_object(obs, name) for name in self.item_names])
    ...
    def get_agent_prompt(self, obs, agent_name):        
        ...
        slot_desp = "\n".join(
            [
                f"{slot_name}: ({x+1.5:.2f}, {y:.2f}, {z:.2f})" for slot_name, (x,y,z) in self.bin_slot_xposes.items()
            ]
            )
        f_ = open(f"./prompt_template/task/{self.env_name}/system_prompt.json", "r", encoding="utf-8")
        agent_template = json.load(f_)["AGENT_PROMPT_TEMPLATE"]
        agent_prompt = agent_template.format(agent_name=agent_name, other_robots=other_robot, slot_desp=slot_desp, object_desp=object_desp, robot_desp=robot_desp, table_height=table_height,)
        return agent_prompt
